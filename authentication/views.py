    ...
    def post(self, request):
        ...
        if not User.objects.filter(username=username).exists():
            ...
            # path to view
                ...
                # - encode uid
                # - token
                uidb64 = urlsafe_base64_encode(force_byte(user.pk))
        ...
        if username and password:
            ...
            if user:
                if user.is_active:
                    ...
                    messages.success(request, 'Wilcome, '+ user.username+ ' you are now logged in')

                    return redirect('expenses')

                    messages.error(request,'Account in not active, please check your email')
                    return render(request, 'authentication/login.html')
